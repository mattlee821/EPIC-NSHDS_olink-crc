...
# associations ====
data_assoc <- data_coloc %>%
  filter(h4 > 0.8) %>%
  mutate(id = paste0(SeqId, ";", exposure_study, ",", cancer, ";", sex))
length(unique(data_assoc$SeqId)) # 49 coloc
unique(data_assoc$SeqId)

data_assoc <- data_mr %>%
  filter(P < 0.05) %>%
  mutate(id = paste0(SeqId, ";", exposure_study, ",", cancer, ";", sex)) %>%
  filter(id %in% data_assoc$id)

# id those not found in epic ====
id <- ID_assoc_epic %>%
  filter(sex == "combined") %>%
  pull(SeqId) %>%
  unique() 
...
data_assoc <- data_assoc[!(data_assoc$SeqId %in% id), ]
data_assoc <- data_assoc %>%
  left_join(data_epic, by = c("SeqId", "cancer", "sex")) %>%
  # select(SeqId, Target.x, TargetFullName.x, cancer, BETA, SE, P, method, coef, se_robust, pval, FDR_BH, exposure_study) %>%
  rename(Target = Target.x,
         TargetFullName = TargetFullName.x)

df1 <- data_assoc %>%
  select(SeqId, Target, TargetFullName, cancer, BETA, SE, P, method, exposure_study, exposure_finemap)
df2 <- data_assoc %>%
  select(SeqId, Target, TargetFullName, cancer, coef, se_robust, FDR_BH) %>%
  rename(BETA = coef,
         SE = se_robust,
         P = FDR_BH) %>%
  mutate(method = "EPIC",
...
data_plot <- bind_rows(df1,df2)

# plot ====
data_plot1 <- subset(data_plot, Target == "Layilin")
data_plot <- subset(data_plot, Target != "Layilin")
p1 <- functions::forestplot(df = data_plot, 
                            name = Target, 
                            estimate = BETA, 
                            se = SE, 
                            pvalue = P, 
...
p2 <- functions::forestplot(df = data_plot1, 
                            name = Target, 
                            estimate = BETA, 
                            se = SE, 
                            pvalue = P, 
  ...
  ggplot2::scale_color_manual(
    values = c(
      "pietzner_2021_PMID34648354" = "purple1",
      "EPIC" = "black"
    )
  ) +
  ggplot2::scale_shape_manual(
...
tiff(filename = "analysis/figures/manuscript/comparison-mr-epic.tiff",
     width = 1600, height = 800, units = "px")
cowplot::plot_grid(p1,p2, ncol = 1, rel_heights = c(1,0.2))
dev.off()
